 ...
 *
 */
public class PCCflowBelowDepth_c extends PCCflow_c implements PCCflowBelowDepth
{
    protected Local var;

    public PCCflowBelowDepth_c(Position pos, Pointcut pc, Local var)  {
        super(pos, pc);
        this.var = var;
    }

    protected PCCflowBelowDepth_c reconstruct(Pointcut pc, Local var) {
        if(pc != this.pc || var != this.var) {
            PCCflowBelowDepth_c ret = (PCCflowBelowDepth_c) copy();
            ret.pc = pc;
            ret.var = var;
            return ret;
        }
	...
	return new abc.weaving.aspectinfo.CflowBelowDepth
    ...
    public Node typeCheck(TypeChecker tc) throws SemanticException {
        ...
        if(!tc.typeSystem().Int().isImplicitCastValid(var.type())) {
            throw new SemanticException("Parameter of cflowbelowdepth must be of type int.");
        }
        return ret;
    }
}
